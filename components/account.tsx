...
export function Account({ section = 'profile' }: { section?: 'history' | 'profile' }) {
 ...
 const profilePanel = <div className="account-grid"><section className="panel lobby-panel profile-editor"><h2>Your profile</h2><div className="form-field"><label htmlFor="profile-name">Display name</label><input className="form-input" id="profile-name" maxLength={40} value={name} onChange={e=>setName(e.target.value)} /></div><div className="form-actions"><button className="button button-primary" disabled={saving || !name.trim()} onClick={save}>Save name</button></div><p role="status">{message}</p></section>
  <section className="panel lobby-panel"><h2>Your stats</h2>{stats ? <><dl className="stats-list"><div><dt>Played</dt><dd>{stats.reduce((n,s)=>n+Number(s.played),0)}</dd></div><div><dt>Won</dt><dd>{stats.reduce((n,s)=>n+Number(s.wins),0)}</dd></div></dl>{stats.map(s=><p key={s.mode}>{s.mode}: {s.wins} wins / {s.played} matches · {s.played ? Math.round(s.wins/s.played*100) : 0}% win rate</p>)}{!stats.length && <p>Finish an online match to start your stats.</p>}</> : <p>Personal stats are not available yet.</p>}<h2>Community leaderboard</h2><p>Ranked by online match wins.</p>{data?.leaderboard.map((p,i)=><div className="account-history-row" key={`${p.display_name}-${i}`}><strong>{i+1}. {p.display_name}</strong><span>{p.wins} wins · {p.games} played</span></div>)}</section></div>;
