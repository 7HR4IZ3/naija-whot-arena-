...
function matchDate(value: string) {
 ...
 return Number.isNaN(date.getTime()) ? 'Recently' : date.toISOString().slice(0, 16).replace('T', ' · ');
}
...
export function Account() {
 ...
 const [tab, setTab] = useState<'history' | 'profile'>('history');
 const { data, error, loading } = useArena<MatchHistory>('history', 'scope', 'me');
 const activeMatches = data?.active ?? [];
 ...
 return <main className="page-wrap"><ScreenHeader title="Your corner of the arena." kicker="ACCOUNT" description="Your name, your rounds, your progress." action={<button className="button button-secondary" onClick={async () => { if(isSupabaseConfigured()) await createClient().auth.signOut(); router.push('/auth'); router.refresh(); }}>Sign out</button>} /><ArenaFeedback error={error} loading={loading} />
  <div className="account-tabs" role="tablist" aria-label="Account sections"><button className={`account-tab ${tab==='history' ? 'is-active' : ''}`} id="account-history-tab" role="tab" aria-selected={tab==='history'} aria-controls="account-history-panel" onClick={()=>setTab('history')}>Match history{activeMatches.length > 0 && <span>{activeMatches.length}</span>}</button><button className={`account-tab ${tab==='profile' ? 'is-active' : ''}`} id="account-profile-tab" role="tab" aria-selected={tab==='profile'} aria-controls="account-profile-panel" onClick={()=>setTab('profile')}>Profile</button></div>
  <div id={tab==='history' ? 'account-history-panel' : 'account-profile-panel'} role="tabpanel" tabIndex={0} aria-labelledby={tab==='history' ? 'account-history-tab' : 'account-profile-tab'}>{tab==='history' ? historyPanel : profilePanel}</div>
 </main>;
}
