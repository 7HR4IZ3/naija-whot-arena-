...
import { botChoice, dealPractice, practiceCanPlay, practiceMove, practiceNextRound, type Difficulty, type PracticeState } from "@/lib/practice-engine";
...
export function PracticeGame(){
 ...
 const locked=!game || game.status!=="running" || game.turn!==0 || moving || Boolean(picker);
 const play=(id:string,suit?:PlayingSuit)=>{
  if(!game || game.status!=="running" || moving || game.turn!==0)return;
 ...
 useEffect(()=>{
  if(!game || game.status!=="running" || game.turn===0 || moving)return;
  const timer=setTimeout(()=>setGame(current=>{if(!current || current.status!=="running" || current.turn===0)return current;const choice=botChoice(current,difficulty);return practiceMove(current,current.players[current.turn].id,choice.cardId,choice.suit);}),750);
 ...
 if(!game || !top)return <main className="page-wrap practice-setup"><Link href="/" className="text-link">Back to play</Link><header className="simple-heading"><h1>Practice</h1><p>A relaxed game. Choose your opponents and start.</p></header><section className="panel lobby-panel practice-options"><div className="form-field"><label htmlFor="practice-count">Players, including you</label><select className="form-select" id="practice-count" value={count} onChange={e=>setCount(Number(e.target.value))}>{[2,3,4].map(n=><option key={n} value={n}>{n} players</option>)}</select></div><div className="form-field"><label htmlFor="practice-difficulty">Difficulty</label><select className="form-select" id="practice-difficulty" value={difficulty} onChange={e=>setDifficulty(e.target.value as Difficulty)}><option value="easy">Easy</option><option value="standard">Standard</option><option value="hard">Hard</option></select></div><div className="practice-start"><button className="button button-primary" onClick={start}>Start practice</button><p>Untimed · No sign-in needed</p></div><RoomSettingsPanel untimed settings={rules} maxPlayers={count} onChange={patch=>setRules(r=>({...r,...patch,turnTimer:"off"}))}/><SavedRules settings={rules} players={count} onLoad={r=>setRules({...r,turnTimer:"off"})}/>{message && <p className="form-error" role="alert">{message}</p>}<Link className="text-link" href="/tutorial">Learn how to play →</Link></section></main>;
 ...
 {game.status==="round-complete" && !moving && <GameModal title={`Knockout round ${game.round} complete`}><p>The highest hand total is eliminated. You can keep watching while the remaining players deal the next round.</p><div className="arena-tender-tally">{(game.knockoutTally||[]).map(player=><div className={`arena-tender-row ${player.eliminated?"is-eliminated":""}`} key={player.id}><span><strong>{player.name}</strong><small>{player.eliminated?"Eliminated · spectating":"Still in"}</small></span><b>{player.score}</b></div>)}</div><p className="arena-tender-result">{game.message}</p><button className="button button-primary" onClick={()=>{setGame(practiceNextRound(game));setDealId(v=>v+1);setSelected(null);}}>Start next round</button></GameModal>}
 {game.status==="finished" && !moving && <GameModal title={game.winner==="0"?"You won!":game.players.find(p=>p.id===game.winner)!.name+" won"}><p>{game.message}</p><RoundResults rounds={game.rounds}/><button className="button button-primary" onClick={start}>Rematch</button><button className="text-link" onClick={()=>setGame(null)}>Change practice settings</button></GameModal>}
 </main>;
}
