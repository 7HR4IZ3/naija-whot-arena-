...
export function TournamentHub() {
 ...
 return <main className="page-wrap page-events"><ScreenHeader title="Tournaments" kicker="TOURNAMENTS" description="Find an event or invite friends to your own." action={<Link className="button button-primary" href="/tournaments/new">Create tournament</Link>} /><ArenaFeedback error={error} loading={loading} /><section className="panel events-panel"><div className="button-row">{['registration','running','finished'].map(f => <button className="button button-secondary" aria-pressed={filter === f} key={f} onClick={() => setFilter(f)}>{f === 'registration' ? 'Open' : f === 'running' ? 'Live' : 'Finished'}</button>)}</div><div className="event-list">{events.filter(e => e.status === filter).map(e => <article className="event-row" key={e.id}><div className="event-copy"><h2>{e.name}</h2><p>{new Date(e.starts_at).toLocaleString()} · Up to {e.max_players} players</p></div><Link className="button button-secondary" href={`/tournaments/${e.id}`}>View event →</Link></article>)}{!loading && !events.some(e => e.status === filter) && <p className="empty-state">No events here yet. Create one and invite your people.</p>}</div></section></main>;
}
