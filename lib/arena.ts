...
export type RoundTallyEntry = { id: string; name: string; score: number; eliminated: boolean };
export type Game = { roomCode?: string; roomHost?: string; recentMoves?: {version:number;message:string}[]; roundResults?: import('@/components/match-extras').RoundResult[]; reactions?: {id:string;name:string;message:string}[]; id: string; version: number; me: string; players: Player[]; hand: Card[]; top: Card; marketCount: number; rules: RoomSettings; turn: number; penalty: number; penaltyType: number; calledSuit: string | null; winner: string | null; status: string; message: string; deadline: string | null; serverTime: string; round: number; tournamentId: string | null; roomId: string | null; tenderTally?: RoundTallyEntry[]; knockoutTally?: RoundTallyEntry[]; event: { type: string; actor?: string; card?: Card; count?: number } };
