...
export type Game = { id: string; version: number; me: string; players: Player[]; hand: Card[]; top: Card; marketCount: number; rules: RoomSettings; turn: number; penalty: number; penaltyType: number; calledSuit: string | null; winner: string | null; status: string; message: string; deadline: string | null; serverTime: string; round: number; tournamentId: string | null; roomId: string | null; event: { type: string; actor?: string; card?: Card; count?: number } };
