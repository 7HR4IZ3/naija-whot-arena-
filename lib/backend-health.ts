...
export async function backendHealth() {
 ...
 try {
  ...
  return { configured: true, database: !error, schemaVersion: data?.schemaVersion ?? null, status: error ? (error.code === 'PGRST202' ? 'migration_required' : 'connection_failed') : 'ready' };
